...
public static class QueryService
{
    ...
    /// <returns>Constructed query string</returns>
    public static string QueryAspNetCore(string url, Microsoft.AspNetCore.Http.Extensions.QueryBuilder queryBuilder)
    {
    ...
    {
        if (queryDictionary.Count is 0) return url;

        var values = queryDictionary.Select(keyValuePair =>
            $"{System.Uri.EscapeDataString(keyValuePair.Key)}={System.Uri.EscapeDataString(keyValuePair.Value)}");
    ...
    {
        if (queryDictionary.QueryLength is 0) return url;
